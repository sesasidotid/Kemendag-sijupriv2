  ...
  constructor(
    ...
    this.activatedRoute.paramMap.subscribe(params => {
      ...
      this.pagable = new PagableBuilder("/api/v1/rw_kinerja/search")
        ...
        .addActionColumn(new ActionColumnBuilder().setAction((rwKinerja: any, event: Event) => {
          const isChecked = (event.target as HTMLInputElement).checked;
          if (isChecked) {
            const exists = this.rwKinerjaList.some(item => item.id === rwKinerja.id);
            if (!exists) {
              this.rwKinerjaList.push(rwKinerja);
            }
          } else {
            const index = this.rwKinerjaList.findIndex(item => item.id === rwKinerja.id);
            if (index !== -1) {
  ...
  setRadarChartData() {
    ...
    this.rwKinerjaList.forEach(rwKinerja => {
      ...
      this.radarChartData.datasets.push({
        ...
        data: [rwKinerja.predikatKinerjaValue, rwKinerja.ratingHasilValue, rwKinerja.ratingKinerjaValue],
        fill: true,
