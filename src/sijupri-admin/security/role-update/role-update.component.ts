...
export class RoleUpdateComponent {
  ...
  constructor(private roleService: RoleService, private menuService: MenuService, private activatedRoute: ActivatedRoute, private router: Router) {
  ...
  submit() {
    ...
    for (const menu of this.menuTree) {
      ...
      if (menu.child) for (const child of menu.child) {
