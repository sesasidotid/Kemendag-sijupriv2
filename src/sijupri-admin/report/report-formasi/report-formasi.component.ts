...
@Component({
    selector: 'app-report-deprecated_formasi',
    standalone: true,
    imports: [
        PagableComponent,
        ReactiveFormsModule,
    ...
    handleSubscribe() {
        this.addFormasiReportForm
            ...
            ?.valueChanges.subscribe((value) => {
                ...
                const dynamicFields = ['unitKerjaId', 'provinsiId', 'kabKotaId']
                            ...
                            next: (result) => {
                                ...
                                this.apiService
                                    ...
                                    .subscribe({
                                        ...
                                        error: (error) =>
                                            this.handlerService.handleException(
                                                error,
                                            ),
                                    })
                            },
        ...
        this.apiService.getData('/api/v1/unit_kerja').subscribe({
            next: (response: any) => {
                this.unitKerjaList = response.map(
                    (unitKerja: { [key: string]: any }) =>
                        new UnitKerja(unitKerja),
                )
            },
            complete: () => {
                loading[0] = true
        ...
        if (this.addFormasiReportForm.valid) {
            this.confirmationService.open(false).subscribe({
                next: (result) => {
                    ...
                    reportGenerate.fileType =
                        this.addFormasiReportForm.value.fileType
                    ...
                    if (
                        this.addFormasiReportForm.value.exportType ==
                        'unitKerja'
                    ) {
                        reportGenerate.parameter = {
                            unitKerjaId:
                                this.addFormasiReportForm.value.unitKerjaId,
                            unitKerjaName:
                                this.addFormasiReportForm.value.unitKerjaName,
                        }
                    }

                    if (
                        this.addFormasiReportForm.value.exportType == 'provinsi'
                    ) {
                        reportGenerate.parameter = {
                            provinsiId:
                                this.addFormasiReportForm.value.provinsiId,
                            provinsiName:
                                this.addFormasiReportForm.value.provinsiName,
                        }
                    }

                    if (
                        this.addFormasiReportForm.value.exportType == 'kabKota'
                    ) {
                        reportGenerate.parameter = {
                            kabKotaId:
                                this.addFormasiReportForm.value.kabKotaId,
                            kabKotaName:
                                this.addFormasiReportForm.value.kabKotaName,
                        }
                    }
