...
export class UkomExamMakalahComponent {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (
            (changes['examDetail'] || changes['roomUkomDetail']) &&
    ...
    getListPertanyaan() {
        const module_id = this.examDetail.examTypeCode

        this.apiService
            .postData(
                `/api/v1/room_ukom/search/${module_id}/${this.roomUkomDetail.id}?limit=1000`,
                {},
            )
            .subscribe({
                next: (res: any) => {
    ...
    onSubmit() {
        this.confirmationService.open(false).subscribe({
            next: ({ confirmed }) => {
                ...
                this.payload = {
                    id: this.roomUkomDetail.id,
                    exam_type_code: this.examDetail.examTypeCode,
