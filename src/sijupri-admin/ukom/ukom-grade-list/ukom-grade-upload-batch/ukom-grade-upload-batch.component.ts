    ...
    uploadRecomendationBatch() {
        this.confirmationService.open(false).subscribe({
            next: ({ confirmed }) => {
                ...
                if (!this.file) {
                    this.handlerService.handleAlert('Error', 'Silakan pilih file terlebih dahulu.');
                    return;
                }

                this.isLoading = true
                this.ukomParticipantService.uploadRecomendationBatch(this.file).pipe(finalize(() => this.isLoading = false))
                    .subscribe({
                        next: () => {
                            this.handlerService.handleAlert('Success', 'File berhasil diunggah!');
                            this.uploadSuccess.emit();
                        },
                        error: (err) => this.handlerService.handleAlert('Error', `Gagal mengunggah: ${err.message}`)
                    });
