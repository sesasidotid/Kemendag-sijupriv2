...
export class UkomExportVerifikasiComponent {
  ...
  handleDownload (reportId: string) {
    this.apiService
      ...
      .subscribe({
        next: () => {
          this.handlerService.handleAlert('Success', 'file di download')
        },
        error: err => {
          this.handlerService.handleAlert('Error', 'Gagal mendownload file')
        }
      })
  }
  ...
  handleDelete (reportId: string) {
    this.confirmationService.open(false).subscribe({
      next: res => {
        ...
        this.apiService.deleteData(`/api/v1/report/${reportId}`).subscribe({
          next: () => {
          ...
          error: err => {
            this.handlerService.handleAlert('Error', 'Gagal menghapus report')
          }
        })
      }
    })
      ...
      next: res => {
        ...
        addIfTruthy(
          ...
          this.jabatanList.find(
            jabatan =>
              jabatan.code == this.hasilVerifikasiForm.get('jabatanCode')?.value
          )?.name
        )
        addIfTruthy(
          'dateFrom',
          this.hasilVerifikasiForm.get('dateFrom')?.value || '2024-01-01'
        )
        addIfTruthy(
          'dateTo',
