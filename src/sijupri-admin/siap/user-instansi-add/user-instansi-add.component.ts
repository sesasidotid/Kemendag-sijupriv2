...
export class UserInstansiAddComponent {
  ...
  getInstansiTypeList() {
    this.apiService.getData(`/api/v1/instansi_type`).subscribe({
      next: (response) => this.instansiTypeList = response.map((instansiType: { [key: string]: any; }) => new InstansiType(instansiType)),
  ...
  getInstansiList() {
    switch (this.operasional) {
      case 'IT1':
        this.apiService.getData(`/api/v1/instansi/type/${this.operasional}`).subscribe({
      ...
      case 'IT2':
        ...
        break;
      case 'IT3':
        this.apiService.getData(`/api/v1/instansi/provinsi/${this.provinsiId}`).subscribe({
          next: (response) => this.instansiList = response.map((instansi: { [key: string]: any; }) => new Instansi(instansi)),
          error: (error) => this.handlerService.handleException(error)
        })
        break;
  ...
  getProvinsiList() {
    this.apiService.getData(`/api/v1/provinsi`).subscribe({
      next: (response) => this.provinsiList = response.map((provinsi: { [key: string]: any; }) => new Provinsi(provinsi)),
      error: (error) => this.handlerService.handleException(error)
    });
  }
  ...
  getKabKotaList() {
    ...
    this.apiService.getData(`/api/v1/kab_kota/type/${type}/${this.provinsiId}`).subscribe({
      next: (response) => this.kabKotaList = response.map((kabKota: { [key: string]: any; }) => new KabKota(kabKota)),
      error: (error) => this.handlerService.handleException(error)
    });
  }
