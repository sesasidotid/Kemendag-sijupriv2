...
@Component({
    ...
    standalone: true,
    imports: [PagableComponent, CommonModule, ModalComponent, BidangJabatanEditComponent, BidangJabatanAddComponent],
...
export class BidangJabatanListComponent {
    ...
    constructor(private router: Router, private tabService: TabService, private apiService: ApiService, private handlerService: HandlerService, private confirmationService: ConfirmationService) { }
    ...
    handleDeleteBidangJabatanByCode(code: string) {
        this.confirmationService.open(false).subscribe({
            next: res => {
                ...
                this.apiService.deleteData(`/api/v1/bidang_jabatan/${code}`).subscribe({
                    next: () => {
                        this.handlerService.handleAlert('Success', 'Berhasil menghapus data bidang jabatan')
                        this.refresh = !this.refresh
                    },
                    error: (error) => {
                        this.handlerService.handleAlert('Error', "Gagal menghapus data bidang jabatan")
                    }
                })
            }
        })
