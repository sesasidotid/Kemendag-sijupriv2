  ...
  getInstrumenList () {
    ...
    this.apiService.getData('/api/v1/instrument').subscribe({
      ...
      error: error => {
        ...
        this.alertService.showToast(
          'Error',
          'Gagal mendapatkan data instrumen!'
        )
        this.instrumentListLoading$.next(false)
      }
    })
  ...
  submit () {
    this.confirmationService.open(false).subscribe({
      next: result => {
        ...
        this.kategoriInstrument.instrumentId =
          this.kknForm.get('instrumentId')?.value
        ...
        this.apiService
          .postData('/api/v1/kategori_instrument', this.kategoriInstrument)
          .subscribe({
            next: () => {
