...
export class AkpPelatihanListComponent {
    ...
    getErrorMessage(controlName: string, label: string): string | null {
        return this.formValidationService.getErrorMessage(this.form.get(controlName), controlName, label);
    }
    ...
    handleDelete(pelatihanId: string) {
        this.confirmationService.open(false).subscribe({
            next: result => {
                ...
                this.apiService
                    .deleteData(`/api/v1/akp_pelatihan_teknis/${pelatihanId}`)
                    .subscribe({
                        next: () => {
                            this.alertService.showToast(
                                'Success',
                                'Berhasil menghapus pelatihan.'
                            )
                            this.tab$.next(null)
                            setTimeout(() => {
                                this.tab$.next(0)
