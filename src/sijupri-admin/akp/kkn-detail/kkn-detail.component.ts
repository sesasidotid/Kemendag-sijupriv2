...
export class KknDetailComponent {
  ...
  getKategoriInstrument () {
    ...
    this.apiService
      .getData(`/api/v1/kategori_instrument/${this.id}`)
      .subscribe({
        next: response => {
          ...
          this.kknForm
            .get('pelatihanTeknisId')
            .patchValue(this.kategoriInstrument.pelatihanTeknisId)
        ...
        error: error => {
          ...
          this.alertService.showToast(
            'Error',
            'Galat saat mengambil data kategori instrument!'
          )
          this.kategoriInstrumentLoading$.next(false)
        }
      })
  ...
  getPelatihanList (instrumentId: string) {
    this.apiService
      .getData(`/api/v1/akp_pelatihan_teknis/instrument/${instrumentId}`)
      .subscribe({
        next: response => {
          this.pelatihanList = response.map(
            (pelatihan: { [key: string]: any }) =>
              new PelatihanTeknis(pelatihan)
          )
        },
        error: error => {
          ...
          this.alertService.showToast(
            'Error',
            'Gagal mendapatkan data pelatihan!'
          )
        }
      })
  }
  ...
  onDelete () {
    this.confirmationService.open(false).subscribe({
      next: result => {
        ...
        this.apiService
          .deleteData(`/api/v1/kategori_instrument/${this.id}`)
          .subscribe({
            next: () => {
              this.router.navigate(['/akp/kkn'])
              this.alertService.showToast(
                'Success',
                'Berhasil menghapus kategori instrument!'
              )
            },
            error: error => {
              console.error('Error fetching data', error)
  ...
  submit () {
    this.confirmationService.open(false).subscribe({
      next: result => {
        ...
        this.kategoriInstrument.pelatihanTeknisId =
          this.kknForm.get('pelatihanTeknisId')?.value

        this.apiService
          .putData('/api/v1/kategori_instrument', this.kategoriInstrument)
          .subscribe({
            next: () => {
              this.router.navigate(['/akp/kkn'])
              this.alertService.showToast(
                'Success',
                'Berhasil mengubah kategori instrument!'
              )
            },
            error: error => {
              console.error('Error fetching data', error)
              this.alertService.showToast(
                'Error',
                'Gagal mengubah kategori instrument!'
              )
            }
          })
      }
