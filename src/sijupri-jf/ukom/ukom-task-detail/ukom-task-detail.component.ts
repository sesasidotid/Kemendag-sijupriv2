...
//                 },
//                 error: err => {
//                     console.error('Failed to load predikat kinerja:', err)
//                 }
//             })
//     }
...
export class UkomTaskDetailComponent implements OnDestroy {
    ...
    private initializeComponent () {
        ...
        this.apiService
            ...
            .pipe(
                ...
                switchMap(res => {
                    console.log('Predikat kinerja loaded:', res)
                    this.predikatKinerjaList = res
                    return this.activatedRoute.paramMap
                }),
                tap(params => {
                    this.id = params.get('id')
                    console.log('ID set from route params:', this.id)
                }),
                finalize(() => {
                    this.isPredikatKerjaLoading$.next(false)
                })
    ...
    getAllScoresFlow (): void {
        console.log('getAllScoresFlow called with ID:', this.id)
        ...
        this.getExamType()
            .pipe(
                ...
                finalize(() => {
                    console.log('Final scoreMap:', this.scoreMap)
                    this.isAllSchoreLoading$.next(false)
                })
            )
            .subscribe({
                next: () => {
                    console.log('getAllScoresFlow completed successfully')
                },
                error: error => {
                    console.error('getAllScoresFlow failed:', error)
                    this.handlerService.handleAlert(
                        'Error',
