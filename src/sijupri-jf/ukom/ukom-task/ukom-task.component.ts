...
export class UkomTaskComponent {
  ...
  getPendingTask() {
    this.apiService.getData(`/api/v1/pending_task/wf_name/peserta_ukom_task/${LoginContext.getUserId()}`).subscribe({
      next: (response) => {
        ...
        this.pesertaUkom.dokumenPesertaUkom.forEach(dokumenPersyaratan => {
          if (dokumenPersyaratan.status == "REJECT") {
            this.inputs.files["dokumenPersyaratan_" + count] = { label: dokumenPersyaratan.dokumenName, source: dokumenPersyaratan.dokumenUrl }
            count = count + 1;
          } else {
