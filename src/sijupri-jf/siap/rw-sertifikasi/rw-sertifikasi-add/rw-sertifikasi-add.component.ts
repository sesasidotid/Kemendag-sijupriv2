...
export class RwSertifikasiAddComponent {
    ...
    fileLoadHandler() {
        this.inputs = {
            files: {
                skPengangkatan: {
                    label: 'Upload Dokumen SK Pengangkatan',
                    fileName: this.rwSertifikasi.skPengangkatan,
                    source: this.rwSertifikasi.skPengangkatanUrl,
                    required: true
                },
                ktpPpns: {
                    label: 'Upload Dokumen KTP PPNS',
                    fileName: this.rwSertifikasi.ktpPpns,
                    source: this.rwSertifikasi.ktpPpnsUrl,
                    visible: () => this.kategori$.value == 2,
                    required: true
                }
            },
            maxSize: 2 * 1024 * 1024,
    ...
    getKategoriSertifikasiList() {
        ...
        this.apiService.getData(`/api/v1/kategori_sertifikasi`).subscribe({
            next: response => {
                this.kategoriSertifikasiList = response.map(
                    (kategoriSertifikasi: { [key: string]: any }) =>
                        new KategoriSertifikasi(kategoriSertifikasi)
                )
                this.rwSertifikasi.kategoriSertifikasiId =
                    this.kategoriSertifikasiList[0].id
            ...
            error: error => {
                ...
                this.alertService.showToast(
                    'Error',
                    'Gagal mendapatkan data kategori sertifikasi!'
                )
            }
        })
    }
    ...
    submit() {
        if (this.rwSertifikasiForm.valid) {
            ...
            if (this.kategori$.value === 1) {
                ...
                this.rwSertifikasi.kategoriSertifikasiName =
                    this.kategoriSertifikasiList[0].name
            }
            if (this.kategori$.value === 2) {
                this.rwSertifikasi.kategoriSertifikasiId =
                    this.kategoriSertifikasiList[1].id
                this.rwSertifikasi.kategoriSertifikasiValue = 2
                this.rwSertifikasi.kategoriSertifikasiName =
                    this.kategoriSertifikasiList[1].name
            }
            ...
            this.confirmationService.open(false).subscribe({
                next: result => {
                    ...
                    this.apiService
                        .postData(`/api/v1/rw_sertifikasi/task`, this.rwSertifikasi)
                        .subscribe({
                            next: () => {
                                this.alertService.showToast(
                                    'Success',
                                    'Berhasil menambahkan riwayat sertifikasi.'
                                )
                            ...
                            error: error => {
                                ...
                                this.alertService.showToast(
                                    'Error',
                                    'Gagal menambahkan riwayat sertifikasi!'
                                )
                            }
                        })
                }
