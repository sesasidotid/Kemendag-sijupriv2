...
export class RwPangkatPendingComponent {
    ...
    fileLoadHandler() {
        this.inputs = {
            files: {
                docEvaluas: {
                    label: 'Upload Dokumen SK Pangkat',
                    fileName: this.rwPangkat.skPangkat,
                    source: this.rwPangkat.skPangkatUrl,
                    required: true
                }
            },
            maxSize: 2 * 1024 * 1024,
        ...
        if (this.rwPangkatForm.valid) {
            ...
            this.confirmationService.open(false).subscribe({
                next: result => {
                    ...
                    this.apiService
                        .postData(`/api/v1/rw_pangkat/task/submit`, task)
                        .subscribe({
                            next: () => {
                                this.alertService.showToast(
                                    'Success',
                                    'Berhasil memperbarui riwayat pangkat.'
                                )
                            ...
                            error: error => {
                                ...
                                this.alertService.showToast(
                                    'Error',
                                    'Gagal memperbarui riwayat pangkat!'
                                )
                            }
                        })
                }
