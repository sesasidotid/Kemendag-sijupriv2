...
export class RwPendidikanPendingComponent {
        ...
        this.inputs = {
            files: {
                ijazah: {
                    ...
                    fileName: this.rwPendidikan.ijazah,
                    source: this.rwPendidikan.ijazahUrl,
                    required: true
                }
            },
            maxSize: 2 * 1024 * 1024,
    ...
    submit () {
        if (this.rwPendidikanForm.valid) {
            ...
            this.rwPendidikan.institusiPendidikan =
                this.rwPendidikanForm.value.institusiPendidikan
            ...
            this.confirmationService.open(false).subscribe({
                next: result => {
                    ...
                    this.apiService
                        .postData(`/api/v1/rw_pendidikan/task/submit`, task)
                        .subscribe({
                            next: () => {
                                this.alertService.showToast(
                                    'Success',
                                    'Berhasil memperbarui pengajuan riwayat pendidikan.'
                                )
