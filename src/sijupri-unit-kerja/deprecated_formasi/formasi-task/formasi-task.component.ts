...
export class FormasiTaskComponent {
    ...
    getPengaturanFormasiJabatan() {
        this.apiService
            .getData(
                `/api/v1/formasi_detail/formasi/${this.pendingTask.objectId}`,
            )
            .subscribe({
                next: (response) => {
                ...
                error: () => {
                    ...
                    this.handlerService.handleAlert(
                        'Error',
                        'Mengambil data pengaturan deprecated_formasi jabatan error',
                    )
                },
            })
    }
    ...
    submitFl1() {
        this.confirmationService.open(false).subscribe({
            next: (result) => {
                ...
                this.apiService
                    .postData('/api/v1/deprecated_formasi/task/submit', {
    ...
    onFileChange(event: any, index: number) {
        ...
        if (file) {
            ...
            reader.onload = () => {
                ...
                this.formasiDokumenList[index].dokumenFile =
        ...
        this.confirmationService.open(false).subscribe({
            next: (response) => {
                ...
                const task = new Task({
                    ...
                    taskAction: 'approve',
                    object: {
                        formasi_dokumen_list: this.revisiedFormasiDokumen,
                    },
                })
