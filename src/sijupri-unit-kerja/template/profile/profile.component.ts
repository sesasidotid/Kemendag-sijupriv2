...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, ProfileCardComponent, ReactiveFormsModule],
...
export class ProfileComponent {
  ...
  getUserUnitKerja() {
    this.userUnitKerjaService.findByNip(this.nip).subscribe({
      ...
      error: () => this.alertService.showToast('Error', "Gagal memperoleh data user unit kerja!")
    })
  }
  ...
  submit() {
    if (this.userUnitKerjaForm.valid) {
      ...
      this.userUnitKerjaService.update(this.userUnitKerja).subscribe({
        next: () => {
          this.alertService.showToast('Success', "Berhasil memperbarui data user unit kerja!");
        },
        error: () => this.alertService.showToast('Error', "Gagal memperbarui data user unit kerja!")
      })
    }
