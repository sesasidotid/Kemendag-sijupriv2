...
import { RWJabatan } from '../models/rw-jabatan.model'
...
export class RwJabatanService {
    readonly BASE_PATH = '/api/v1/rw_jabatan'
    ...
    findAll(): Observable<RWJabatan[]> {
        return this.apiService.getData(this.BASE_PATH).pipe(
            map((response: any) => {
                return response.map(
                    (jabatan: { [key: string]: any }) => new RWJabatan(jabatan),
                )
            }),
            catchError((error) => {
    ...
    findByNip(id: string): Observable<RWJabatan> {
        return this.apiService.getData(`${this.BASE_PATH}/${id}`).pipe(
            map((response: any) => {
                return new RWJabatan(response)
            }),
            catchError((error) => {
                console.error('Error fetching data', error)
                this.alertService.showToast('Error', error.error.message)
    ...
    detailObject(object_task_id: string): Observable<ObjectTask> {
        return this.apiService
            .getData(`${this.BASE_PATH}/task/${object_task_id}`)
            .pipe(
                map((response: any) => {
    ...
    saveTask(user: RWJabatan): Observable<void> {
        return this.apiService.postData(`${this.BASE_PATH}/task`, user).pipe(
            catchError((error) => {
                console.error('Error save RWJabatan', error)
                this.alertService.showToast('Error', error.error.message)
                throw error
            }),
        )
    ...
    updateTask(user: RWJabatan): Observable<void> {
        return this.apiService.putData(`${this.BASE_PATH}/task`, user).pipe(
            catchError((error) => {
                console.error('Error update RWJabatan', error)
                this.alertService.showToast('Error', error.error.message)
                throw error
            }),
        )
