...
export class FileHandlerComponent {
  ...
  constructor(private filePreviewService: FilePreviewService, private fileConverterService: FileConverterService) { }
  ...
  ngOnInit() {
    ...
    for (const key in this.inputs.files) {
      if (this.inputs.files[key].fileName && this.inputs.files[key].source) {
        ...
        this.fileConverterService.getFileAsBase64(this.inputs.files[key].source).subscribe({
          next: (base64) => {
            this.inputs.listen(key, base64, base64, this.inputs.files[key].label);
          },
          complete: () => {
