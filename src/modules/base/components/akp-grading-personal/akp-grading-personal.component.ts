...
export class AKPGradingPersonalComponent {
  ...
  createForm() {
    ...
    this.AKPGrading.dataPertanyaanList.forEach(question => {
      ...
      controls[question.pertanyaanId] = new FormGroup(
        questionControls,
        this.atLeastOneZeroValidator(question.pertanyaanId.toString(), [controlPenugasan, controlMateri, controlInformasi])
      );
  ...
  atLeastOneZeroValidator(questionId: string, keys: string[]): ValidatorFn {
    return (group: AbstractControl): ValidationErrors | null => {
      ...
      if (controls[`${questionId}_nilaiYbs`]?.value + controls[`${questionId}_nilaiAtasan`]?.value + controls[`${questionId}_nilaiRekan`].value < 7) {
