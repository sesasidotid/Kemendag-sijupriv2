...
export class PageFilterBuilder {
    ...
    withField(lable: string, fieldType: "text" | "date" | "select"): PageFilterBuilder {
        this.lable = lable;
    ...
    setOptionList(optionList: { label: string, value: string | number | boolean }[]): PageFilterBuilder {
        this.optionList = optionList;
        return this;
    }

    setOptionListFromObjectList(objectList: any[], labelProperty: string, valueProperty: string): PageFilterBuilder {
        this.optionList = [];
    ...
    setProperty(property: string, parentsProperty: string[] = []): PageFilterBuilder {
    ...
    build(): PageFilter {
        return new PageFilter({ label: this.lable, fieldType: this.fieldType, value: this.value, key: `${this.clauses[this.clause]}${this.property}`, optionList: this.optionList })
    }
}
...
export class PrimaryColumnBuilder {
    ...
    withPropertyValue(label: string, property: string, parentsProperty?: string[]): PrimaryColumnBuilder {
        this.label = label;
        if (parentsProperty)
            parentsProperty.forEach(pProp => {
                this.property = `${pProp}|` + this.property;
    ...
    withDefaultValue(defaultValue: string | number | boolean | null = null): PrimaryColumnBuilder {
        this.defaultValue = defaultValue;
        return this;
    }

    build(): PageColumn {
        return new PageColumn({ columnType: "primary", label: this.label, property: this.property, dynamic: this.dynamic, defaultValue: this.defaultValue })
    }
}
...
export class ActionColumnBuilder {
    ...
    build(): PageColumn {
        return new PageColumn({ columnType: "action", process: this.process, icon: this.icon, color: this.color, inactive: this.inactive })
    }
}