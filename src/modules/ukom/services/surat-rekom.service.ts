...
export class SuratRekomService {
    ...
    previewSuratRekom(): Observable<{baseTemplate: string, template: string}> {
        return this.apiService.getData(`${this.BASE_PATH}/template/REKOM_UKOM`).pipe(
            catchError((err) => {
                console.error(err)
                this.handlerService.handleException(err)
                throw err
    ...
    setupSuratRekom(payload: CreatePreviewSuratRekomRequest): Observable<string> {
        const body = {
            code: 'REKOM_UKOM',
            parameters: {
                bulan: payload.bulan,
