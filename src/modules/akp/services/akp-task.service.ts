...
export class AkpTaskService {
    ...
    saveTask(id: string): Observable<any> {
        return this.apiService
            .postData(`${this.BASE_PATH_TASK}`, { nip: id })
            .pipe(
                catchError((error) => {
    ...
    getAKPByReviewer(id: string, whoIs: string): Observable<AKPByReviewer> {
        return this.apiService
            .getData(`${this.BASE_PATH_AKP}/reviewer/${whoIs}/${id}`)
            .pipe(
                map((response: any) => {
    ...
    getAKPByPersonal(id: string): Observable<AKPByPersonal> {
        return this.apiService
            .getData(`${this.BASE_PATH_AKP}/personal/${id}`)
            .pipe(
                map((response: any) => {
    ...
    saveAKPReviewBySelf(review: any): Observable<any> {
        return this.apiService
            .postData(`${this.BASE_PATH_AKP}/matrix/personal`, review)
            .pipe(
                catchError((error) => {
                    console.error('error', error)
    ...
    saveAKPReviewByBoss(review: any): Observable<any> {
        return this.apiService
            .postData(`${this.BASE_PATH_AKP}/matrix/atasan`, review)
            .pipe(
                catchError((error) => {
                    console.error('error', error)
                    // this.alertService.showToast('Error', error.error.message);
    ...
    saveAKPReviewByColleague(review: any): Observable<any> {
        return this.apiService
            .postData(`${this.BASE_PATH_AKP}/matrix/rekan`, review)
            .pipe(
                catchError((error) => {
                    console.error('error', error)
                    // this.alertService.showToast('Error', error.error.message);
    ...
    verifAKPTask(payload: VerifAKPTask): Observable<any> {
        return this.apiService
            .postData(`${this.BASE_PATH_TASK}/submit`, payload)
            .pipe(
                catchError((error) => {
                    console.error('error', error)
                    // this.alertService.showToast('Error', error.error.message);
