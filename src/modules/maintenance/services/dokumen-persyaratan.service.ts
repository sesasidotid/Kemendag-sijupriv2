...
export class DokumenPersyaratanService {
    ...
    findAll(): Observable<DokumenPersyaratan[]> {
        return this.apiService.getData(this.BASE_PATH).pipe(
            map((response: any) => {
                return response.map(
                    (dokumenPersyaratan: { [key: string]: any }) =>
                        new DokumenPersyaratan(dokumenPersyaratan),
                )
            }),
            catchError((error) => {
    ...
    findByAssociation(association: string): Observable<DokumenPersyaratan[]> {
        return this.apiService
            .getData(`${this.BASE_PATH}/association/${association}`)
            .pipe(
                map((response: any) => {
                    return response.map(
                        (dokumenPersyaratan: { [key: string]: any }) =>
    ...
    save(dokumenPersyaratan: DokumenPersyaratan): Observable<void> {
        return this.apiService
            .postData(this.BASE_PATH, dokumenPersyaratan)
            .pipe(
                catchError((error) => {
                    console.error('Error fetching data', error)
                    this.alertService.showToast('Error', error.error.message)
