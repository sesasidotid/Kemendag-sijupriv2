...
export class FormasiService {
  ...
  detailTaskJabatan (
    unitKerjaId: string,
    jabatanCode: string
  ): Observable<ObjectTask> {
    return this.apiService
      .getData(`${this.BASE_PATH}/task/${unitKerjaId}/${jabatanCode}`)
      .pipe(
        map((response: any) => {
  ...
  calculate (unsurList: any): Observable<FormasiResult[]> {
    return this.apiService
      .postData(`${this.BASE_PATH}/calculate`, unsurList)
      .pipe(
        map((response: any) => {
          return response.map(
            (formasiUnsur: { [key: string]: any }) =>
              new FormasiResult(formasiUnsur)
          )
        }),
        catchError(error => {
